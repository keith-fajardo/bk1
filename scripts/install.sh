#!/usr/bin/env bash
#
# bk1 installer — builds the Rust lint sidecar binary, installs it into the
# bk1 skill directory, and installs the Bun dependencies.
#
...
# cargo is soft-required: skip the Rust build if cargo is missing AND a bk1-lint binary
# already exists at the destination. Lets asset-only updates ship without forcing every
# contributor to install the Rust toolchain.
